...
const AdminCommunity: NextPage = ({ initialInquiry, ...props }: any) => {
	...
	/** APOLLO REQUESTS **/
	const [updateArticleByAdmin] = useMutation(UPDATE_BOARD_ARTICLE_BY_ADMIN);
	const [removeArticleByAdmin] = useMutation(REMOVE_BOARD_ARTICLE_BY_ADMIN);
	...
	const removeArticleHandler = async (id: string) => {
		try {
			if (await sweetConfirmAlert('are you sure to remove?')) {
				await removeArticleByAdmin({ variables: { input: id }})
				await getAllBoardArticlesByAdminRefetch({ input: communityInquiry })
			}
		} catch (err: any) {
			sweetErrorHandling(err).then();
