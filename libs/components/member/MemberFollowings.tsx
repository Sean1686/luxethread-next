...
const MemberFollowings = (props: MemberFollowingsProps) => {
	...
	const targetMemberId =
		typeof router.query.memberId === 'string' ? router.query.memberId : user?._id || '';
	...
	useEffect(() => {
		...
		setFollowInquiry((prev) => ({ ...prev, search: { followerId: targetMemberId } }));
	}, [targetMemberId]);
							...
							<Stack className="follows-card-box" key={follower._id}>
								...
								<Stack className={'details-box'}>
									...
									<Box 
										className={'info-box'} 
										component={'div'}
										onClick={() => likeMemberHandler(followInquiry, getMemberFollowingsRefetch, follower?.followingData?._id)}
										sx={{ cursor: 'pointer' }}
									>
